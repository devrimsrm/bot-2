...
exports.run = async (client, message, args) => {
  ...
  if (
    ![client.config.vmutehammer].some(role =>
  ...
  if (!muteler.some(j => j.id == kullanici.id)) {
    ...
    mutelog.send(
      new MessageEmbed()
        ...
        .setTimestamp()
        .setDescription(
          `${kullanici} (${member.user.tag} - ${
            member.user.id
          }) kişisi ${zaman1} boyunca metin kanallarında susturuldu. \n
          • Susturulma sebebi: **${sebep}**
          • Susturulma Tarihi:  **${moment(Date.now())
            .add(10, "hours")
            .format("HH:mm:ss DD MMMM YYYY")}
